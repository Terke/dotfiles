...
case $desktop in
    ...
    if type "xrandr" > /dev/null; then
      for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
        MONITOR=$m polybar --reload top_bar -c ~/.config/polybar/config &
      done
    else
    polybar --reload top_bar -c ~/.config/polybar/config &
    fi
    # second polybar at bottom
     if type "xrandr" > /dev/null; then
       for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
         MONITOR=$m polybar --reload bottom_bar -c ~/.config/polybar/config &
       done
     else
     polybar --reload bottom_bar -c ~/.config/polybar/config &
     fi
    ;;
